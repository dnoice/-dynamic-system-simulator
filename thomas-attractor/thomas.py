# -*- coding: utf-8 -*-
"""
Thomas' Attractor Simulator, Analyzer, and Visualizer (PEP 8 Compliant)

This script simulates Thomas' cyclically symmetric attractor, a chaotic system
known for its intricate, pretzel-like structure.

It performs the following actions:
1.  **Defines Thomas' System:** Implements the system of three ODEs using sine functions.
2.  **Solves ODEs:** Integrates the Thomas equations using SciPy's odeint.
...
Usage:
  ...
  Outputs saved in 'thomas_output' directory.
"""
...
# --- Output Settings ---
OUTPUT_DIR_NAME: str = "thomas_output"
VIDEO_ANIMATION_FILENAME: str = "thomas_animation.mp4"
INTERACTIVE_HTML_FILENAME: str = "thomas_interactive.html"
STATIC_PLOT_FILENAME: str = "thomas_static_plots.png"
LLE_RESULTS_FILENAME: str = "thomas_lle_results.txt"

# --- Thomas' System Parameter (Classic Chaotic Regime) ---
THOMAS_PARAM_B: float = 0.19 # A common value yielding chaos

# --- Initial States ---
INITIAL_STATES: List[List[float]] = [
    [1.0, 0.5, -0.5], # A typical starting point
    [1.01, 0.5, -0.5],
    [1.0, 0.51, -0.5],
]

# --- Simulation Time Spans (Thomas often requires longer integration) ---
VIS_T_END: float = 400.0       # End time for visualization
VIS_NUM_STEPS: int = 80000     # Number of steps for visualization
LLE_T_END: float = 400.0       # End time for LLE calculation
LLE_NUM_STEPS: int = 90000     # Number of steps for LLE calculation
...
# Adjust FPS/Interval/FrameStep for Thomas' visual speed
ANIMATION_FPS: int = 30 # Standard FPS
ANIMATION_DPI: int = 100 # Standard DPI
FRAME_STEP: int = 25 # Sample points less frequently for visual speed
TAIL_LENGTH: int = 300 # Tail length in number of *points*
ANIMATION_INTERVAL: int = 33 # Approx 30 FPS (1000ms / 30fps)

# --- Visualization Settings ---
STATIC_PLOT_DPI: int = 150 # Reduced DPI slightly from original for faster save

# Type alias for state vector
State = np.ndarray
# Type alias for system parameters tuple (only 'b' for Thomas)
ThomasParams = Tuple[float]

# =====================================
# Function Definitions
# =====================================

def thomas_system(state: State, t: float, b: float) -> State:
    """
    Defines the differential equations for Thomas' cyclically symmetric attractor.
    ...
    """
    x, y, z = state
    # Using np.sin for potentially faster computation on arrays if needed later
    dxdt = np.sin(y) - b * x
    dydt = np.sin(z) - b * y
    dzdt = np.sin(x) - b * z
...
def solve_visual_trajectories(
    ...
    sys_params: Tuple, # Expects a tuple, e.g., (b,) for Thomas
) -> List[np.ndarray]:
    """
    ...
    Args:
        ...
        system_func: Function defining the ODE system (e.g., thomas_system).
...
# --- save_lle_results (Adapted for Thomas parameter 'b') ---
def save_lle_results(
    filepath: pathlib.Path,
    params: ThomasParams, # Takes the (b,) tuple
    sim_time: float,
    target_time: float,
    num_steps: int,
    epsilon: float,
    ...
    """
    Saves LLE calculation results and Thomas parameter to a text file.

    Args:
        filepath: Path object for the output text file.
        params: Tuple containing Thomas parameter (b,).
    ...
    param_b = params[0] # Extract b from the tuple
    try:
        with open(filepath, "w", encoding='utf-8') as f:
            f.write("=" * 30 + "\n")
            f.write(" Thomas' Attractor Analysis Results\n")
            f.write("=" * 30 + "\n\n")
...
# --- generate_save_static_plots (Adapted for Thomas) ---
def generate_save_static_plots(
    ...
    """
    Generates and saves static plots (3D and 2D projections) of Thomas' attractor.
    ...
    title = f"Thomas' Attractor (b={param_b:.3f}){title_lle_part}"
    ...
    cmap_3d = 'magma' # Colormap
    linewidth_3d = 0.4; alpha_3d = 0.6 # Thinner line for dense Thomas plot
    ...
    for i, solution in enumerate(solutions):
        ...
        lc = Line3DCollection(segments, cmap=cmap_3d, norm=norm, linewidth=linewidth_3d, alpha=alpha_3d)
    ...
    ax3d.scatter(starts[:, 0], starts[:, 1], starts[:, 2], color='cyan', s=70, marker='o', edgecolor='black', label='Start Points', depthshade=False, zorder=10)
    ax3d.legend(loc='upper left')

    # Set axis limits based on data, maybe slightly wider than default range
    limit_max = max(np.abs(all_x).max(), np.abs(all_y).max(), np.abs(all_z).max()) * 1.1
    ax3d.set_xlim(-limit_max, limit_max)
    ax3d.set_ylim(-limit_max, limit_max)
    ax3d.set_zlim(-limit_max, limit_max)

    ax3d.view_init(elev=30, azim=45) # Adjust view angle

    x_p, y_p, z_p = solutions[0][:, 0], solutions[0][:, 1], solutions[0][:, 2]
    scatter_args = {'c': t_span, 'cmap': 'cividis', 's': 0.2, 'alpha': 0.5} # Small points for dense plot
...
# --- generate_save_animation (Adapted for Thomas) ---
def generate_save_animation(
    ...
    """
    Generates and saves an animation of Thomas' attractor trajectory.
    ...
    Args:
        ...
        writer_name: Name of Matplotlib animation writer. None skips saving.
    ...
    line_anim, = ax_anim.plot([], [], [], lw=1.2, color='orange') # Line color
    point_anim, = ax_anim.plot([], [], [], 'o', color='blue', markersize=5, zorder=10) # Marker color

    limit_max = max(np.abs(x_anim).max(), np.abs(y_anim).max(), np.abs(z_anim).max()) * 1.1
    ax_anim.set_xlim(-limit_max, limit_max); ax_anim.set_ylim(-limit_max, limit_max); ax_anim.set_zlim(-limit_max, limit_max)
    ax_anim.set_xlabel("X Axis"); ax_anim.set_ylabel("Y Axis"); ax_anim.set_zlabel("Z Axis")
    ax_anim.set_title("Thomas' Attractor Animation")
    ...
    def update(frame_idx_scaled: int) -> Tuple[matplotlib.lines.Line2D, matplotlib.lines.Line2D]:
        ...
        line_anim.set_data(x_anim[tail_start_data_idx : current_data_idx+1], y_anim[tail_start_data_idx : current_data_idx+1])
        line_anim.set_3d_properties(z_anim[tail_start_data_idx : current_data_idx+1])
        point_anim.set_data(x_anim[current_data_idx : current_data_idx+1], y_anim[current_data_idx : current_data_idx+1])
...
# --- generate_save_interactive_html (Adapted for Thomas) ---
def generate_save_interactive_html(
    ...
    """
    Generates and saves an interactive 3D plot of Thomas' attractor using Plotly.
    ...
    try:
        trace = go.Scatter3d(
            ...
            line=dict(color=t_span, colorscale='Magma', width=2.5, # Slightly thicker line
                      colorbar=dict(title='Time')),
            name='Thomas Trajectory'
        )

        # Determine symmetric range based on max absolute value
        limit_max = max(np.abs(x_trace).max(), np.abs(y_trace).max(), np.abs(z_trace).max()) * 1.1
        axis_range = [-limit_max, limit_max]

        layout = go.Layout(
            title=dict(text=f"Interactive Thomas' Attractor<br>(b={param_b:.3f})", x=0.5, xanchor='center'),
            scene=dict(
                xaxis_title='X Axis', yaxis_title='Y Axis', zaxis_title='Z Axis',
                xaxis=dict(backgroundcolor="rgb(230, 230, 230)", gridcolor="white", zerolinecolor="white", range=axis_range),
                yaxis=dict(backgroundcolor="rgb(230, 230, 230)", gridcolor="white", zerolinecolor="white", range=axis_range),
                zaxis=dict(backgroundcolor="rgb(230, 230, 230)", gridcolor="white", zerolinecolor="white", range=axis_range),
                aspectratio=dict(x=1, y=1, z=1), # Equal aspect ratio often looks good
...
def open_file_cross_platform(filepath: pathlib.Path) -> None:
    ...
    if termux_open_cmd:
        ...
        try:
            ...
            termux_used = True
        except FileNotFoundError: print(f"  [!] Error: '{termux_open_cmd}' not found during execution.")
        except subprocess.CalledProcessError as e: print(f"  [!] Error: 'termux-open' failed (code {e.returncode})."); print(f"    Stderr: {e.stderr.strip()}") if e.stderr else None
        except subprocess.TimeoutExpired: print("  [!] Error: 'termux-open' command timed out.")
        except Exception as e: print(f"  [!] Unexpected error executing 'termux-open': {e}")

    if not termux_used:
        if termux_open_cmd: print("  [*] 'termux-open' failed, falling back to standard webbrowser...")
        else: print("  [*] 'termux-open' not found. Using standard webbrowser...")
        try:
            opened = webbrowser.open(file_uri)
            if opened: print(f"  [+] Standard webbrowser requested opening: {file_uri}")
            else: print(f"  [!] Standard webbrowser reported failure to open."); print(f"      Platform: {platform.system()}. Try opening manually: {filepath.resolve()}")
        except webbrowser.Error as e: print(f"  [!] Error using webbrowser module: {e}"); print(f"      Try opening manually: {filepath.resolve()}")
        except Exception as e: print(f"  [!] Unexpected error using webbrowser: {e}"); print(f"      Try opening manually: {filepath.resolve()}")
...
def main() -> int:
    """
    Main function to orchestrate Thomas' Attractor simulation and analysis.

    Returns:
        0 if successful, 1 otherwise.
    """
    print("=" * 60)
    print(" Thomas' Attractor Simulation, Analysis, and Visualization")
    print("=" * 60)
    ...
    # Use the single parameter 'b' for Thomas, ensure it's passed as a tuple
    thomas_params: ThomasParams = (THOMAS_PARAM_B,)
    t_span_vis = np.linspace(0.0, VIS_T_END, VIS_NUM_STEPS)
    t_span_lle = np.linspace(0.0, LLE_T_END, LLE_NUM_STEPS)
    print(f"[*] System Parameter: b={thomas_params[0]:.3f}")
    ...
    try:
        solutions = solve_visual_trajectories(INITIAL_STATES, t_span_vis, thomas_system, thomas_params)
    except Exception as e:
    ...
    lle_estimate, lle_interpretation, lle_sim_time = calculate_lle(
        INITIAL_STATES[0], t_span_lle, thomas_system, thomas_params, LLE_EPSILON
    )

    # --- Save LLE Results ---
    # Pass thomas_params tuple directly
    save_lle_results(
        lle_results_path, thomas_params,
    ...
    generate_save_static_plots(
        ...
        thomas_params, lle_estimate, STATIC_PLOT_DPI,
    )
    ...
    html_saved = generate_save_interactive_html(
        interactive_html_path, first_solution, t_span_vis, thomas_params
    )
    ...
    print("\n[+] Thomas' Attractor script finished successfully.")
    print("=" * 60)
    return 0
